...
b8 game_on_button(u16 code, void* sender, void* listener_list, event_context context) {
    if (code == EVENT_CODE_BUTTON_PRESSED) {
        ...
        case MOUSE_BUTTON_LEFT: {
            ...
            application_state* state = (application_state*)listener_list;
...
b8 application_initialize(struct application* game_inst) {
    ...
    kaudio_master_volume_set(state->audio_system, 0.7f);
...
b8 application_update(application* game_inst, struct frame_data* p_frame_data) {
    ...
    if (state->running) {
        ...
        // Keep a running average of update and render timers over the last ~1 second.秒
        ...
        total_update_seconds += state->update_clock.elapsed;
...
b8 application_prepare_frame(struct application* app_inst, struct frame_data* p_frame_data) {
    ...
    for (u32 i = 0;i < node_count;++i) {
        ...
        if (strings_equali(node->name, "sui")) {
            ...
            forward_rendergraph_node_camera_projection_set(node,
                current_camera,
            ...
            if (dir_light) {
                ...
                for (u32 c = 0; c < MATERIAL_MAX_SHADOW_CASCADES; c++) {
                    ...
                    mat4 light_ortho = mat4_orthographic(extents.min.x, extents.max.x, extents.min.y, extents.max.y, extents.min.z, extents.max.z - extents.min.z);
            ...
            debug_rendergraph_node_view_projection_set(node,
...
void application_unregister_events(struct application* game_inst) {
    ...
    event_unregister(EVENT_CODE_DEBUG5, game_inst, game_on_debug_event);
