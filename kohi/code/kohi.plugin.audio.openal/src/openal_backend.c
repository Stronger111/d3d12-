...
typedef struct kresource_work_thread_params {
    kaudio_backend_interface* backend;
    kaudio_plugin_source* source;
}kresource_work_thread_params;
...
static u32 openal_backend_find_free_buffer(kaudio_backend_interface* backend);
static const char* openal_backend_error_str(ALenum err);
static b8 openal_backend_check_error(void);
...
b8 openal_backend_initialize(kaudio_backend_interface* backend, const kaudio_backend_config* config) {
    if (backend) {
        ...
        state->buffer_count = 245; // FIXME: load from config.
        ...
        KINFO("OpenAL plugin initialized");
...
b8 openal_backend_resource_load(kaudio_backend_interface* backend, const kresource_audio* resource, b8 is_stream, khandle resource_handle) {
    ...
    if (is_stream) {
        ...
        //Non-streamed only need one buffer.
        ...
        openal_backend_check_error();
        if (data->total_samples_left > 0) {
            //Load the whole thing into the buffer.DXS
            alBufferData(data->mono_buffer, AL_FORMAT_MONO16, (i16*)resource->mono_pcm_data, data->total_samples_left, resource->sample_rate);
            openal_backend_check_error();
        }
        //Non-streamed do not loop by default.
...
b8 openal_backend_channel_play_resource(kaudio_backend_interface* backend, khandle resource_handle, kaudio_space audio_space, u8 channel_id) {
    ...
    if (data->is_stream) {
        ...
        //If a sound is currently playing here,clip it off by stoping first.
...
static b8 stream_resource_data(kaudio_backend_interface* backend, ALuint buffer, kaudio_space audio_space, kaudio_resource_data* resource) {
    ...
    //Get the data offset.
    i16* streamed_data = data_source + pos;// audio->stream_buffer_data(audio);
    if (streamed_data) {
...
static u32 source_work_thread(void* params) {
    kresource_work_thread_params* typed_params = params;
    ...
    kfree(params, sizeof(kresource_work_thread_params), MEMORY_TAG_AUDIO);
...
static b8 openal_backend_channel_create(kaudio_backend_interface* backend, kaudio_plugin_source* out_source) {
    ...
    kresource_work_thread_params* params = kallocate(sizeof(kresource_work_thread_params), MEMORY_TAG_AUDIO);
...
static u32 openal_backend_find_free_buffer(kaudio_backend_interface* backend) {
    if (backend) {
        ...
        //If there are no free buffer, attempt to free one first.
        if (free_count == 0) {
