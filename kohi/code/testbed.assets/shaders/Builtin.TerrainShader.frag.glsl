...
float geometry_schlick_ggx(float normal_dot_direction,float roughness){
      roughness+=1.0;
      float k=(roughness*roughness)/8;  //direction k的系数  IBL r平方/2
...
void main() {
    ...
    vec4 frag_position_view_space=global_ubo.views[u_push_constants.view_index]*vec4(in_dto.frag_position,1.0); //片段在view空间的位置
...
vec3 calculate_reflectance(vec3 albedo,vec3 normal, vec3 view_direction,vec3 light_direction,float metallic,float roughness,vec3 base_reflectivity,vec3 radiance){
    ...
    // Geometry function which calculates self-shadowing on micro-facets (more pronounced on rough surfaces). 计算自阴影和微观平面
    ...
    float geometry=ggx_0*ggx_1;

    // Fresnel-Schlick approximation for the fresnel. This generates a ratio of surface reflection base_reflectivity 根据材料特性
    ...
    float denominator=4.0*max(dot(normal,view_direction),0.0)+0.0001; //防止除0
