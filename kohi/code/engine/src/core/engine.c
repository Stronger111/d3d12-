...
b8 engine_create(application* game_inst) {
    ...
    event_register(EVENT_CODE_OBJECT_HOVER_ID_CHANGED, 0, engine_on_event);
    ...
    if (!platform_system_startup(&engine_state->platform_system_memory_requirement,
    ...
    i32 thread_count = platform_get_processor_count();
    if (thread_count < 1) {
