...
typedef struct shadow_map_pass_internal_data {
    ...
    b8* instance_updated;
    u32 instance_count;
    // Default map to be used when materials arenot available.
...
b8 shadow_map_pass_initialize(rendergraph_pass* self) {
    ...
    shadowmap_pass_config.target.attachments = kallocate(sizeof(render_target_attachment_config) * shadowmap_pass_config.target.attachment_count, MEMORY_TAG_RENDERER);
...
b8 shadow_map_pass_load_resources(rendergraph_pass* self) {
    ...
    internal_data->default_terrain_colour_map.repeat_u = internal_data->default_colour_map.repeat_v = internal_data->default_colour_map.repeat_w = TEXTURE_REPEAT_CLAMP_TO_EDGE;
    internal_data->default_terrain_colour_map.filter_minify = internal_data->default_colour_map.filter_magnify = TEXTURE_FILTER_MODE_LINEAR;
...
b8 shadow_map_pass_execute(rendergraph_pass* self, frame_data* p_frame_data) {
    ...
    // Terrain will be slightly different since a texture sample isnot
    // really need since terrains are never transparent. Therefor, only
    // one more instance is needed, which can use the same default white
    // texture as a sample.
    highest_id++;

    if (highest_id > internal_data->instance_count) {
        // Get more resource if needed,starting at the previous high point.
    ...
    for (u32 i = 0; i < geometry_count; ++i) {
        ...
        if (g->material && g->material->maps) {
            // Use current materials internal id.
            // NOTE: +1 to account for the first id being taken by the default instance.
