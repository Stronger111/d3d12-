...
b8 vulkan_material_shader_create(vulkan_context* context, vulkan_material_shader* out_shader) {
    ...
    VkDescriptorType descriptor_types[VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT] = {
        ...
        VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,  // Binding 1 - Texture sampler
    };
    ...
    object_pool_sizes[1].descriptorCount = VULKAN_MAX_MATERIAL_COUNT * VULKAN_MATERIAL_SHADER_SAMPLER_COUNT;
...
#define ATTRIBUTE_COUNT 2
    ...
    VkDescriptorSetLayout layouts[2] = {out_shader->global_descriptor_set_layout, out_shader->object_descriptor_set_layout};
...
void vulkan_material_shader_apply_material(vulkan_context* context, vulkan_material_shader* shader, material* material) {
    if (context && shader) {
        ...
        // f32 s = (ksin(accumulator) + 1.0f) / 2.0f;  // scale from -1 ,0 to 1
        ...
        for (u32 sampler_index = 0; sampler_index < sampler_count; ++sampler_index) {
            ...
            // If the texture hasnot been loaded yet, use the default
