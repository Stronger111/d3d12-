...
// Debugger
    ...
    context.in_flight_fences = darray_reserve(VkFence, context.swapchain.max_frames_in_flight);
    ...
    // by this list
    context.images_in_flight = darray_reserve(VkFence, context.swapchain.image_count);
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend* backend, f32 delta_time) {
    ...
    if (context.recreating_swapchain) {
        ...
        if (!vulkan_result_is_success(result)) {
            KERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle(1) failed:'%s'", vulkan_result_to_string(result, true));
            return false;
        }
    }
...
b8 vulkan_renderer_backend_end_frame(renderer_backend* backend, f32 delta_time) {
    ...
    if (result != VK_SUCCESS) {
        KERROR("vkQueueSubmit failed with result:%s", vulkan_result_to_string(result, true));
        return false;
    }
...
void vulkan_renderer_create_texture(const u8* pixels, texture* texture) {
    ...
    if (!vulkan_result_is_success(VK_SUCCESS)) {
        KERROR("Error creating texture sampler: %s", vulkan_result_to_string(result, true));
        return;
    }
    texture->generation++;
}
...
void regenerate_framebuffers(renderer_backend* backend,
    ...
    for (u32 i = 0; i < swapchain->image_count; ++i) {
        // TODO: make this dynamic based on the currently confiured attachments
