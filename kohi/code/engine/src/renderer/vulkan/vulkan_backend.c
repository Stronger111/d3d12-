...
b8 upload_data_range(vulkan_context* context, VkCommandPool pool, VkFence fence, VkQueue queue, vulkan_buffer* buffer,
                     u64* out_offset, u64 size, const void* data) {
...
// Debugger
    ...
    vulkan_renderpass_create(&context, &context.main_renderpass, (vec4){0, 0, context.framebuffer_width, context.framebuffer_height},
                             (vec4){0.0f, 0.0f, 0.2f, 1.0f},
                             1.0f,
                             0,
                             RENDERPASS_CLEAR_COLOUR_BUFFER_FLAG | RENDERPASS_CLEAR_DEPTH_BUFFER_FLAG | RENDERPASS_CLEAR_STENCIL_BUFFER_FLAG, false, true);
...
b8 vulkan_renderer_begin_renderpass(renderer_backend* backend, u8 renderpass_id) {
    ...
    // Render Initial 创建CommandBuffer
    vulkan_command_buffer* command_buffer = &context.graphics_command_buffers[context.image_index];
...
void vulkan_renderer_create_texture(const u8* pixels, texture* texture) {
    ...
    vulkan_buffer staging;
    vulkan_buffer_create(&context, image_size, usage, memory_property_flags, true, false, &staging);
...
b8 vulkan_renderer_create_geometry(geometry* geometry, u32 vertex_size, u32 vertex_count, const void* vertices, u32 index_size, u32 index_count, const void* indices) {
    ...
    if (!upload_data_range(&context, pool, 0, queue, &context.object_vertex_buffer, &internal_data->vertex_buffer_offset, total_size, vertices)) {
...
b8 vulkan_renderer_shader_create(shader* shader, u8 renderpass_id, u8 stage_count, const char** stage_filenames, shader_stage* stages) {
    ...
    out_shader->config.descriptor_set_count = 1;
    // 单个实例进行更新
...
b8 vulkan_renderer_shader_acquire_instance_resources(shader* s, texture_map** maps, u32* out_instance_id) {
    ...
    *out_instance_id = INVALID_ID;
    ...
    instance_state->instance_texture_maps = kallocate(sizeof(texture_map*) * instance_texture_count, MEMORY_TAG_ARRAY);
