...
b8 vulkan_buffer_create(vulkan_context* context,
    ...
    out_buffer->memory_index = context->find_memory_index(memory_requirements.memoryTypeBits, out_buffer->memory_property_flags);
...
void vulkan_buffer_copy_to(vulkan_context* context, VkCommandPool pool, VkFence fence, VkQueue queue, VkBuffer source, u64 source_offset, VkBuffer dest, u64 dest_offset, u64 size) 
{
    ...
    vkCmdCopyBuffer(temp_command_buffer.handle, source, dest, 1, &region);
