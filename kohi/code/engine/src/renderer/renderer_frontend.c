...
void renderer_shader_destroy(shader* s) {
    return state_ptr->plugin.shader_destroy(s);
}
...
void renderer_texture_map_release_resources(texture_map* map) {
    return state_ptr->plugin.texture_map_release_resources(map);
}
...
void renderer_renderbuffer_unmap_memory(renderbuffer* buffer, u64 offset, u64 size) {
    return state_ptr->plugin.renderbuffer_unmap_memory(buffer, offset, size);
}
