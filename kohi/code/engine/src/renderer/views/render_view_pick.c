...
b8 render_view_pick_on_create(struct render_view* self) {
    if (self) {
        ...
        // Extract uniform locations.
        data->terrain_shader_info.id_colour_location = shader_system_uniform_index(data->world_shader_info.s, "id_colour");
        data->terrain_shader_info.model_location = shader_system_uniform_index(data->world_shader_info.s, "model");
        data->terrain_shader_info.projection_location = shader_system_uniform_index(data->world_shader_info.s, "projection");
        data->terrain_shader_info.view_location = shader_system_uniform_index(data->world_shader_info.s, "view");
...
void render_view_pick_on_resize(render_view* self, u32 width, u32 height) {
    render_view_pick_internal_data* data = self->internal_data;
...
b8 render_view_pick_on_packet_build(const render_view* self, struct linear_allocator* frame_allocator, void* data, render_view_packet* out_packet) {
    ...
    // Iterate all geometries in the terrain data
...
void render_view_pick_on_packet_destroy(const render_view* self, render_view_packet* packet) {
    darray_destroy(packet->geometries);
    darray_destroy(packet->terrain_geometries);
    kzero_memory(packet, sizeof(render_view_packet));
}

b8 render_view_pick_on_render(const render_view* self, const render_view_packet* packet, u64 frame_number, u64 render_target_index, const struct frame_data* p_frame_data) {
    render_view_pick_internal_data* data = self->internal_data;
