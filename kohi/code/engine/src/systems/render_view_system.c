...
b8 render_view_system_register(render_view* view) {
    if (!view) {
        KERROR("render_view_system_register requires a pointer to a valid config.");
        return false;
    }
    ...
    if (id == INVALID_ID_U16) {
        KERROR("render_view_system_registered - No available space for a new view. Change system config to account for more.");
        return false;
    }
    ...
    // Set the array elements pointer
