...
static b8 image_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource) {
    ...
    image_resource_params* type_params = (image_resource_params*)params;

    char* format_str = "%s/%s/%s%s";
    const i32 required_channel_count = 4;
    stbi_set_flip_vertically_on_load_thread(type_params->flip_y);
...
//stbi_info_from_memory 从内存中查看图片的属性 当数组纹理中
//所有的纹理都加载完毕 组成TextureArray 
