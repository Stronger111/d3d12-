...
b8 ui_rendergraph_node_execute(struct rendergraph_node* self, struct frame_data* p_frame_data) {
    ...
    for (u32 i = 0; i < renderable_count; ++i) {
        ...
        // Apply group
        {
            ...
            //Set UBO data
            sui_per_group_ubo group_ubo_data = { 0 };
            group_ubo_data.diffuse_colour = renderable->render_data.diffuse_colour;
            shader_system_uniform_set_by_location(internal_data->sui_shader, internal_data->sui_locations.sui_group_ubo, &group_ubo_data);
            //Atlas texture
