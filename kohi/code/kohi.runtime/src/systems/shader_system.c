...
static b8 internal_sampler_add(shader* shader, const shader_uniform_config* config);
...
static b8 file_watch_event(u16 code, void* sender, void* listener_inst, event_context context) {
    ...
    if (code == EVENT_CODE_WATCHED_FILE_WRITTEN) {
        ...
        // Seach shaders for the one with the changed file watch id.
...
shader* shader_system_get(const char* shader_name) {
    ...
    //Attempt once more to get a  shader_id.
...
b8 shader_system_bind_group(u32 shader_id, u32 group_id) {
    if (group_id == INVALID_ID) {
        KERROR("Cannot bind shader group INVALID_ID.");
        return false;
    }
...
b8 shader_system_bind_draw_id(u32 shader_id, u32 draw_id) {
    if (draw_id == INVALID_ID) {
        KERROR("Cannot bind shader draw id INVALID_ID.");
        return false;
    }
...
b8 shader_system_shader_group_acquire(u32 shader_id, u32 map_count, kresource_texture_map** maps, u32* out_instance_id) {
    ...
    if (config->frequency == SHADER_UPDATE_FREQUENCY_PER_FRAME) {
        ...
        *map = default_map;
        ...
        if (shader->per_group_texture_count + 1 > state_ptr->config.max_per_group_textures) {
            KERROR("Shader per-group texture count %i exceeds max of %i", shader->per_group_texture_count, state_ptr->config.max_per_group_textures);
            return false;
        }
...
static b8 internal_uniform_add(shader* shader, const shader_uniform_config* config, u32 location) {
    ...
    if (!is_sampler) {
        if (entry.frequency == SHADER_UPDATE_FREQUENCY_PER_FRAME) {
            ...
            shader->per_draw_ubo_size += (entry.size * entry.array_length);  // local_ubo_size PushConstant shader类型
        }
    }
