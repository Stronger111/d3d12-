...
typedef struct kshader {
    ...
    /** @brief The types of topologies used by the shader and its pipeline. See primitive_topology_type_bits. */
    u32 topology_types;
...
static khandle shader_create(const kresource_shader* shader_resource) {
    ...
    // Invalidate frequency bounds ids
    ...
    for (u8 i = 0; i < shader_resource->stage_count; ++i) {
        ...
        //Take of a copy of the generation for later comparison.
...
static b8 shader_reload(kshader* shader, khandle shader_handle) {
    //Check each shade stage generation for out-of-sync.
