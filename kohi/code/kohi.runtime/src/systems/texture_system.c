...
b8 texture_system_initialize(u64* memory_requirement, void* state, void* config) {
    ...
    // Block of memory will contain state structure,the block for array,then block for hashtable.
    ...
    *memory_requirement = struct_requirement + array_requirement + hashtable_requirement;
    ...
    // Hashtable block is after array. +纹理需要多少内存空间
    void* hashtable_block = array_block + array_requirement;

    // Create a hashtable with invalid
    ...
    hashtable_fill(&state_ptr->registered_texture_table, &invalid_ref);  // Key其实是地址？
...
texture* texture_system_acquire_textures_as_arrayed(const char* name, u32 layer_count, const char** layer_texture_names, b8 auto_release) {
    ...
    texture* t = &state_ptr->registered_textures[id];
    // Create it,if needed. width 0 height 0 ???
    if (needs_creation) {
...
static b8 texture_load_layered_job_start(void* params, void* result_data) {
    ...
    // LEFTOFF:Split this into 3 jobs - The first job performs the query, then sets up all of the layerd texture jobs, then sets
