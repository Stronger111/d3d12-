...
struct khandle texture_system_resource_get_internal_or_default(const kresource_texture* t, u32* out_generation) {
    if (!t || !out_generation) {
...
static b8 create_default_textures(texture_system_state* state) {
    ...
    {
        u32 layer_size = sizeof(u8) * 16 * 16 * 4;  // 每一层像素占用大小
        ...
        for (u32 i = 0; i < terrain_material_count; ++i) {
            // Albedo NOTE: purposefully using checkboard here instead of default diffuse white.
...
static void release_default_textures(texture_system_state* state) {
    if (state) {
        ...
        kresource_system_release(state->kresource_system, (kresource*)state->default_kresource_diffuse_texture->base.name);
        ...
        kresource_system_release(state->kresource_system, (kresource*)state->default_kresource_combined_texture->base.name);
        kresource_system_release(state->kresource_system, (kresource*)state->default_kresource_cube_texture->base.name);
        kresource_system_release(state->kresource_system, (kresource*)state->default_kresource_terrain_texture->base.name);
    }
}
...
static b8 texture_load_layered_job_start(void* params, void* result_data) {
    ...
    // Copy relevant properties from the orignal texture.
    ...
    // Create a threadpool to load layers simultanously
    threadpool pool = { 0 };
...
static b8 load_texture(const char* texture_name, texture* t, const char** layer_names) {
    ...
    else if (t->type == TEXTURE_TYPE_2D_ARRAY) {  // 纹理数组
            ...
            if (reference_diff < 0) {
                ...
                if (ref.handle == INVALID_ID) {
                    ...
                    if (*out_texture_id == INVALID_ID) {
                        ...
                        // Set some basic properties on the texture
                        texture* t = &state_ptr->registered_textures[ref.handle];
                        t->id = ref.handle;
