...
static kresource_texture* request_writable_arrayed(kname name, u32 width, u32 height, texture_format format, b8 has_transparency, texture_type type, u16 array_size,
    b8 is_depth, b8 include_stencil, b8 multiframe_buffering);
...
static void combine_asset_pixel_data(kasset_image** assets, u32 count, u32 expected_with, u32 expected_height, b8 release_assets, u32* out_size, void** out_pixels);
...
ktexture* texture_acquire_with_options(ktexture_load_options options, void* listener, PFN_texture_loaded_callback callback) {
    ...
    //If any entry with the name exists,return it.
    if (t) {
    ...
    if (image_asset_names) {
        ...
        context->texture = t;
        context->image_asset_name = image_asset_names;
...
ktexture* texture_acquire_with_options_sync(ktexture_load_options options) {
    ...
    if (!t->mip_levels) {
        t->mip_levels = calculate_mip_levels_from_dimensions(t->width, t->height);
    }
...
kresource_texture* texture_system_request_cube(kname name, b8 auto_release, b8 multiframe_buffering, void* listener, PFN_resource_loaded_user_callback callback) {
    ...
    if (name == state->default_kresource_cube_texture->base.name) {
        return state_ptr->default_kresource_cube_texture;
    }
...
kresource_texture* texture_system_request_cube_writeable(kname name, u32 dimension, b8 auto_release, b8 multiframe_buffering) {
    ...
    return request_writable_arrayed(name, dimension, dimension, TEXTURE_FORMAT_RGBA8, false, TEXTURE_TYPE_CUBE, 6, false, false, multiframe_buffering);
}
...
kresource_texture* texture_system_request_cube_depth(kname name, u32 dimension, b8 auto_release, b8 include_stencil, b8 multiframe_buffering) {
    ...
    return request_writable_arrayed(name, dimension, dimension, TEXTURE_FORMAT_RGBA8, false, TEXTURE_TYPE_CUBE, 6, true, include_stencil, multiframe_buffering);
}
...
kresource_texture* texture_system_request_writeable(kname name, u32 width, u32 height, texture_format format, b8 has_transparency, b8 multiframe_buffering) {
    return request_writable_arrayed(name, width, height, format, has_transparency, TEXTURE_TYPE_2D, 1, false, false, multiframe_buffering);
}
...
kresource_texture* texture_system_request_writeable_arrayed(kname name, u32 width, u32 height, texture_format format, b8 has_transparency, b8 multiframe_buffering, texture_type type, u16 array_size) {
    return request_writable_arrayed(name, width, height, format, has_transparency, type, array_size, false, false, multiframe_buffering);
}
...
kresource_texture* texture_system_request_depth(kname name, u32 width, u32 height, b8 include_stencil, b8 multiframe_buffering) {
    return request_writable_arrayed(name, width, height, TEXTURE_FORMAT_RGBA8, false, TEXTURE_TYPE_2D, 1, true, include_stencil, multiframe_buffering);
}
...
kresource_texture* texture_system_request_depth_arrayed(kname name, u32 width, u32 height, u16 array_size, b8 include_stencil, b8 multiframe_buffering) {
    return request_writable_arrayed(name, width, height, TEXTURE_FORMAT_RGBA8, false, TEXTURE_TYPE_2D_ARRAY, array_size, true, include_stencil, multiframe_buffering);
}
...
static b8 is_default_texture(texture_system_state* state, kresource_texture* t) {
    ...
    return (t == state_ptr->default_kresource_texture) ||
        (t == state_ptr->default_kresource_base_colour_texture) ||
        (t == state_ptr->default_kresource_normal_texture) ||
        (t == state_ptr->default_kresource_specular_texture) ||
        (t == state_ptr->default_kresource_mra_texture) ||
        (t == state_ptr->default_kresource_water_normal_texture) ||
...
static void increment_generation(kresource_texture* t) {
    if (t) {
        t->base.generation++;
        //Ensure we don't land on invalide before rolling over.
