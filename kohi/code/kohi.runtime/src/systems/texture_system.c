...
ktexture texture_acquire_with_options(ktexture_load_options options, void* listener, PFN_texture_loaded_callback callback) {
    ...
    //Ensure there is always at least 1 layer.
    ...
    if (options.pixel_data && options.pixel_array_size) {
        ...
        //Write the imaghe asset data to the texture.
...
static b8 create_default_textures(texture_system_state* state) {
    ...
    // Default texture
    {
        ...
        ktexture t = texture_acquire_from_pixel_data(KPIXEL_FORMAT_RGBA8, pixel_array_size, default_pixels, 16, 16, kname_create(DEFAULT_TEXTURE_NAME));
