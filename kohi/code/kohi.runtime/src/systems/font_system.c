...
b8 create_system_font_variant(system_font_lookup* lookup, u16 size, kname font_name, system_font_variant_data* out_variant);
b8 rebuild_system_font_variant_atlas(system_font_lookup* lookup, system_font_variant_data* variant);
b8 verify_system_font_size_variant(system_font_lookup* lookup, system_font_variant_data* variant, const char* text);
...
static b8 generate_font_geometry(const font_data* font, font_type type, const char* text, font_geometry* pending_data);
...
static vec2 measure_string(font_data* font, const char* text) {
    ...
    for (u32 c = 0; c < char_length; ++c) {
        ...
        // Continue to next line from newline.
        if (codepoint == '\n') {
            if (x > extents.x) {
                extents.x = x;
            }
