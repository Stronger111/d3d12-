...
b8 load_material(material_system_state* state, material_config* config, material* m);
...
material* material_system_acquire_terrain_material(const char* material_name, u32 material_count, const char** material_names, b8 auto_release) {
    ...
    if (needs_creation) {
        ...
        // One map is needed for the entire material array.
        {
            ...
            // 数组纹理
            kresource_texture_map* map = &m->maps[SAMP_TERRAIN_MATERIAL_ARRAY_MAP];  // 0
        ...
        material_textures->kresource_texture_map_count = 1;
        material_textures->kresource_texture_maps = kallocate(sizeof(kresource_texture_map*) * material_textures->kresource_texture_map_count, MEMORY_TAG_ARRAY);
        material_textures->kresource_texture_maps[0] = &m->maps[SAMP_TERRAIN_MATERIAL_ARRAY_MAP];
        ...
        ibl_cube_texture->kresource_texture_maps = kallocate(sizeof(texture_map*) * ibl_cube_texture->kresource_texture_map_count, MEMORY_TAG_ARRAY);
    ...
    if (config->type == MATERIAL_TYPE_PBR) {
        ...
        for (u32 i = 0; i < instance_sampler_count; ++i) {
            ...
            for (u32 j = 0; j < uniform_config->texture_map_count; ++j) {
                uniform_config->kresource_texture_maps[j] = &m->maps[map_offset + i];
            }
        }
    }
    else {
...
static b8 create_default_terrain_material(material_system_state* state) {
   ...
   // Setup a configuration to get instance resources for this material.
    ...
    for (u32 i = 0; i < instance_resource_config.uniform_config_count; ++i) {
        shader_instance_uniform_texture_config* ucfg = &instance_resource_config.uniform_configs[i];
        kfree(ucfg->kresource_texture_maps, sizeof(kresource_texture_map) * ucfg->texture_map_count, MEMORY_TAG_ARRAY);
        ucfg->texture_maps = 0;
    }
    kfree(instance_resource_config.uniform_configs, sizeof(shader_instance_uniform_texture_config) * instance_resource_config.uniform_config_count, MEMORY_TAG_ARRAY);
