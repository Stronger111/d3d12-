...
typedef struct material_system_state {
    ...
    ktexture default_texture; //这个textureHandle为0吗
       ...
       //bind per-group
        ...
        if (mra_enabled) {
            ...
            if (base_material->ao_texture && FLAG_GET(base_material->flags, KMATERIAL_FLAG_AO_ENABLED_BIT)) {
                ...
                shader_system_texture_set_by_location_arrayed(shader, state->standard_material_locations.material_textures, MAT_STANDARD_IDX_NORMAL, base_material->ao_texture);
            }
            else {
...
static b8 create_default_standard_material(material_system_state* state) {
    ...
    asset.ambient_occlusion_enabled = MATERIAL_DEFAULT_AO_VALUE;
...
static b8 material_create(material_system_state* state, khandle material_handle, const kasset_material* asset) {
    ...
    if (material->type == KMATERIAL_TYPE_STANDARD) {
        ...
        if (asset->dudv_map.resource_name) {
            material->dudv_texture = texture_acquire_from_package(asset->dudv_map.resource_name, asset->dudv_map.package_name, 0, 0);
        }
        else {
            material->dudv_texture = texture_acquire_from_package(kname_create(DEFAULT_WATER_DUDV_TEXTURE_NAME), state->runtime_package_name, 0, 0);
        }
...
static material_data* get_material_data(material_system_state* state, khandle material_handle) {
    ...
    if (khandle_is_invalid(material_handle)) {
        KWARN("Attempted to get material data with an invalid base material or instance handle. Nothing to do.");
        return 0;
    }
...
static b8 material_on_event(u16 code, void* sender, void* listener_inst, event_context context) {
    if (code == EVENT_CODE_WINDOW_RESIZED) {
        ...
        //TODO: Scale texture to be smaller based on some global settings.
