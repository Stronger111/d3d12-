...
void destroy_material(kresource_material* m);
...
/* static b8 load_material(material_system_state* state, material_config* config, material* m) {
    ...
    if (config->type == MATERIAL_TYPE_PBR) {
        ...
        for (u32 i = 0; i < PBR_MATERIAL_MAP_COUNT; ++i) {
            mat_maps_assigned[i] = false;
        }
        b8 ibl_cube_assigned = false;
        const char* map_names[PBR_MATERIAL_MAP_COUNT] = { "albedo", "normal", "combined" };
        const kresource_texture* default_textures[PBR_MATERIAL_MAP_COUNT] = {
        ...
        for (u32 i = 0; i < configure_map_count; ++i) {
            b8 found = false;
            for (u32 tex_slot = 0; tex_slot < PBR_MATERIAL_MAP_COUNT; ++tex_slot) {
        ...
        mat_textures->kresource_texture_map_count = PBR_MATERIAL_MAP_COUNT;
