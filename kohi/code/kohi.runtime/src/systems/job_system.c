...
typedef struct job_thread {
    ...
    // Used to casuse a thread to block until work is available.
...
static u32 job_thread_run(void* params) {
    ...
    while (true) {
        ...
        if (info.entry_point) {
            ...
            // Note that store_result takes a copy of the result_data 结果是副本 所以当前线程就可以释放掉结果数据
...
b8 job_system_initialize(u64* job_system_memory_requirement, void* state, void* config) {
    ...
    *job_system_memory_requirement = sizeof(job_system_state) + (sizeof(u8) * (INVALID_ID_U16 - 1));
    ...
    state_ptr->thread_count = type_config->max_job_thread_count;
    ...
    for (u8 i = 0; i < state_ptr->thread_count; ++i) {
        ...
        state_ptr->job_threads[i].type_mask = type_config->type_masks[i];
