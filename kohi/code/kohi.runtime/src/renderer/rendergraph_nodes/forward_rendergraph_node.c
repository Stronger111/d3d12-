...
b8 forward_rendergraph_node_create(struct rendergraph* graph, struct rendergraph_node* self, const struct rendergraph_node_config* config) {
    ...
    //Grab sink configs first,These are all required because they need source linkage.
...
b8 forward_rendergraph_node_initialize(struct rendergraph_node* self) {
    ...
    internal_data->pbr_locations.material_textures = shader_system_uniform_location(internal_data->pbr_shader_id, "material_textures");
    ...
    internal_data->terrain_locations.material_textures = shader_system_uniform_location(internal_data->terrain_shader_id, "material_textures");
...
b8 render_scene(forward_rendergraph_node_internal_data* internal_data, kresource_texture* colour, kresource_texture* depth, u32 plane_count, water_plane** planes, b8 include_water_plane, vec4 clipping_plane, camera* cam, camera* inverted_cam, b8 use_inverted, struct frame_data* p_frame_data) {
    ...
    if (terrain_geometry_count > 0) {
        ...
        // Apply globals
        {
            ...
            //Apply/upload them to GPU
            if (!shader_system_apply_per_frame(internal_data->terrain_shader->id)) {
        ...
        for (u32 i = 0; i < terrain_geometry_count; ++i) {
            ...
            //Apply instance.
            {
                ...
                UNIFORM_APPLY_OR_FAIL(shader_system_uniform_set_by_location(internal_data->terrain_shader_id, internal_data->terrain_locations.material_textures, &m->maps[TERRAIN_SAMP_IDX_MATERIAL_ARRAY_MAP]));
        ...
        if (geometry_count > 0) {
            ...
            for (u32 i = 0; i < count; ++i) {
                ...
                if (m->internal_id != current_material_id) {
                    ...
                    //Maps
                    UNIFORM_APPLY_OR_FAIL(shader_system_uniform_set_by_location_arrayed(internal_data->pbr_shader_id, internal_data->pbr_locations.material_textures, PBR_SAMP_IDX_ALBEDO, &m->maps[PBR_SAMP_IDX_ALBEDO]));
                    UNIFORM_APPLY_OR_FAIL(shader_system_uniform_set_by_location_arrayed(internal_data->pbr_shader_id, internal_data->pbr_locations.material_textures, PBR_SAMP_IDX_NORMAL, &m->maps[PBR_SAMP_IDX_NORMAL]));
...
b8 forward_rendergraph_node_dynamic_geometries_set(struct rendergraph_node* self, struct frame_data* p_frame_data, u32 geometry_count, const struct geometry_render_data* geometries) {
    return true;
}
