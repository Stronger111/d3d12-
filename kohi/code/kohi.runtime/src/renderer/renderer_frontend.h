 ...
 */
KAPI b8 renderer_kresource_texture_resources_acquire(struct renderer_system_state* state, kname name, kresource_texture_type type, u32 width, u32 height, u8 channel_count, u8 mip_level, u16 array_size, kresource_texture_flag_bits flags, khandle* out_renderer_texture_handle);
 ...
 */
KAPI void renderer_texture_resources_release(struct renderer_system_state* state, khandle* renderer_texture_handle);
 ...
 * @returns True on success; otherwise false.
 */
KAPI b8 renderer_texture_read_data(struct renderer_system_state* state, khandle renderer_texture_handle, u32 offset, u32 size, u8** out_pixels);
