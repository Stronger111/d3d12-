...
static void recalculate_points(debug_line3d* line) {
    if (line) {
        colour_vertex_3d* vert = (colour_vertex_3d*)line->geometry.vertices;
        vert[0].position =vec4_from_vec3(line->point_0, 1.0f);
        vert[1].position =vec4_from_vec3(line->point_1, 1.0f);
    }
}
