...
b8 ibl_probe_load(ibl_probe* probe) {
    if (!probe) {
        KERROR("ibl_probe_load requires a valid pointer to probe.");
        return false;
    }
    ...
    probe->ibl_texture = texture_system_request_cube(probe->cubemap_name, true, false, 0, 0);
    return true;
}

void ibl_probe_unload(ibl_probe* probe) {
    if(probe->ibl_texture){
      texture_system_release_resource((kresource_texture*)probe->ibl_texture);
        probe->ibl_texture = 0;
    }
}