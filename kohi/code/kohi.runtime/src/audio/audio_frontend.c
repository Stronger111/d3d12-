        ...
        for (u32 i = 0;i < state->audio_channel_count;++i) {
            ...
            if (channel->bound_audio != INVALID_KAUDIO && channel->bound_instance != INVALID_ID_U16) {
                ...
                if (instance->trigger_play && is_valid) {
                    b8 play_result = state->backend->channel_play_resource(state->backend, channel->bound_audio, instance->audio_space, channel->index);
...
b8 kaudio_play_in_category(struct kaudio_system_state* state, kaudio_instance instance, i8 category_index) {
