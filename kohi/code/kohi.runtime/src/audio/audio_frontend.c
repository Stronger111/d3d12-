...
typedef struct kaudio_resource_instance_data {
    ...
    f32 inner_radius;
    // The radius around the position where the sound is no longer audible.
    f32 outer_radius;
...
typedef struct kaudio_resource_handle_data {
    ...
    //darray of instance of this resource.
...
static void kaudio_emitter_update(kaudio_system_state* state, kaudio_emitter_handle_data* emitter);
...
b8 kaudio_system_initialize(u64* memory_requirement, void* memory, const char* config_str) {
    ...
    *memory_requirement = sizeof(kaudio_system_state);
    ...
    for (u32 i = 0; i < state->category_count; ++i) {
        ...
        for (u32 i = 0;i < state->audio_channel_count;++i) {
            ...
            if (channel->bound_resource && channel->bound_instance) {
                ...
                if (channel->bound_instance->trigger_play && is_valid) {
                    b8 play_result = state->backend->channel_play_resource(state->backend, channel->bound_resource->resource->internal_resource, channel->bound_instance->audio_space, channel->index);
                ...
                // Volume
                f32 gain = 1.0f;
                //Applay the volume at various levels by mixing them.
...
b8 kaudio_acquire(struct kaudio_system_state* state, kname resource_name, kname package_name, b8 is_streaming, kaudio_space audio_space, audio_instance* out_audio_instance) {
    ...
    if (!data->resource) {
        ...
        //Create the darray for the instance.
...
void kaudio_release(struct kaudio_system_state* state, audio_instance* instance) {
    if (state && instance) {
        ...
        //See how many active instances there are left. If one,release.
        ...
        if (!active_instance_count) {
            ...
            //Reset the handle data and make the slot avaliable for use.
...
b8 kaudio_play_in_category(struct kaudio_system_state* state, audio_instance instance, i8 category_index) {
...
b8 kaudio_emitter_create(struct kaudio_system_state* state, f32 inner_radius, f32 outer_radius, f32 volume, f32 falloff, b8 is_looping, b8 is_streaming,
