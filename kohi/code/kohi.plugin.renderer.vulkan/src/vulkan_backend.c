...
//     vulkan_shader* internal_shader = (vulkan_shader*)s->internal_data;
    ...
    if (!context->device.features.fillModeNonSolid) {
        KINFO("Renderer backend does not support fillModeNonSolid. Wireframe mode is not possible,but was requested for the kshader '%s'.", config->name);
    ...
    // Process attributes
    u32 attribute_count = darray_length(config->attributes);
    u32 offset = 0;
    ...
    if (!shader_create_modules_and_pipelines(backend, internal_shader, config->stage_count, config->stage_configs)) {
        KERROR("Failed initial load on kshader '%s'. See logs for details.", config->name);
        return false;
    }
...
b8 vulkan_renderer_shader_apply_per_draw(renderer_backend_interface* backend, khandle shader, u64 renderer_frame_number) {
    ...
    vkCmdPushConstants(command_buffer, pipeline_array[internal_shader->bound_pipeline_index]->pipeline_layout,
