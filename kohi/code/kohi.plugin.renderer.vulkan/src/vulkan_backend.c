...
b8 vulkan_renderer_backend_initialize(renderer_backend_interface* backend, const renderer_backend_config* config) {
    ...
    rhi->kvkEnumerateInstanceExtensionProperties(0, &avaliable_extension_count, 0);
    VkExtensionProperties* available_extensions = darray_reserve(VkExtensionProperties, avaliable_extension_count);
    rhi->kvkEnumerateInstanceExtensionProperties(0, &avaliable_extension_count, available_extensions);
    ...
    if (context->validation_enabled) {
        ...
        if (context->debug_messenger) {
            PFN_vkDestroyDebugUtilsMessengerEXT func =
                (PFN_vkDestroyDebugUtilsMessengerEXT)rhi->kvkGetInstanceProcAddr(
...
b8 vulkan_renderer_frame_command_list_end(renderer_backend_interface* backend, struct frame_data* p_frame_data) {
    ...
    //the current swapchain image. Start by transitioning to transfer source layout.
    {
        ...
        rhi->kvkCmdPipelineBarrier(command_buffer->handle, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, 0, 0, 0, 1, &barrier);
        VK_END_DEBUG_LABEL(context, command_buffer->handle);
    }

    //Trasnition the swapchain image to transfer destination layout.
    {
        VK_BEGIN_DEBUG_LABEL(context, command_buffer->handle, "swapchain image->transfer_dst", vec4_one());
        VkImageMemoryBarrier barrier = { 0 };
        barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
        ...
        rhi->kvkCmdPipelineBarrier(command_buffer->handle, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, 0, 0, 0, 1, &barrier);
        VK_END_DEBUG_LABEL(context, command_buffer->handle);
    }
    ...
    //Now do the blit operation from the source image to the target image.
    {
        ...
        blit_region.srcOffsets[0] = (VkOffset3D){ 0, 0, 0 };  //Starting coordiates in the source image.
        ...
        blit_region.dstOffsets[0] = (VkOffset3D){ 0, 0, 0 };  //Starting coordiates in the target image.
        blit_region.dstOffsets[1] = (VkOffset3D){ target_image->width,target_image->height, 1 }; //Ending coordinates in the target image.
        ...
        rhi->kvkCmdBlitImage(command_buffer->handle,
    ...
    {
        VK_BEGIN_DEBUG_LABEL(context, command_buffer->handle, "window colour->att.pt", vec4_one());
        VkImageMemoryBarrier barrier = { 0 };
        barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
        ...
        barrier.subresourceRange.baseMipLevel = 0;
        barrier.subresourceRange.levelCount = source_image->mip_levels;
        rhi->kvkCmdPipelineBarrier(command_buffer->handle, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0, 0, 0, 0, 1, &barrier);
        VK_END_DEBUG_LABEL(context, command_buffer->handle);
    }
    ...
    //Transition target for presentation.
    {
        ...
        barrier.subresourceRange.baseMipLevel = 0;
        barrier.subresourceRange.levelCount = target_image->mip_levels;
        // After the image has been blitted.
        rhi->kvkCmdPipelineBarrier(command_buffer->handle, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, 0, 0, 0, 1, &barrier);
        VK_END_DEBUG_LABEL(context, command_buffer->handle);
    }
...
void vulkan_renderer_clear_colour_texture(renderer_backend_interface* backend, khandle renderer_texture_handle) {
    ...
    //Clear the image.
    rhi->kvkCmdClearColorImage(command_buffer->handle, image->handle,
...
void vulkan_renderer_clear_depth_stencil(renderer_backend_interface* backend, khandle renderer_texture_handle) {
    ...
    //Clear the image.
    rhi->kvkCmdClearDepthStencilImage(command_buffer->handle, image->handle,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    // Transition to depth/stencil attachment optimal layout for rendering.
    {
        ...
        rhi->kvkCmdPipelineBarrier(
            command_buffer->handle,
            VK_PIPELINE_STAGE_TRANSFER_BIT,
            VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT, //Early Z 优化?
            0,
            0, 0,
            0, 0,
            1, &barrier);
...
b8 vulkan_renderer_shader_create(renderer_backend_interface* backend, khandle shader, const kresource_shader* shader_resource) {
    ...
    //Enusre all frequencies are "unbound"
   ...
   // a.k.a. push constant stride to that, and only ever use one.
    ...
    if (!context->device.features.fillModeNonSolid) {
        KINFO("Renderer backend does not support fillModeNonSolid. Wireframe mode is not possible,but was requested for the kshader '%s'.", kname_string_get(shader_resource->base.name));
    ...
    if (!shader_create_modules_and_pipelines(backend, internal_shader, shader_resource->stage_count, shader_resource->stage_configs)) {
        KERROR("Failed initial load on kshader '%s'. See logs for details.", kname_string_get(shader_resource->base.name));
        return false;
    }
...
b8 vulkan_renderer_shader_apply_per_frame(renderer_backend_interface* backend, khandle shader, u16 renderer_frame_number) {
    ...
    //Per-frame is always first, if exists.
...
b8 vulkan_renderer_shader_apply_per_draw(renderer_backend_interface* backend, khandle shader, u16 renderer_frame_number) {
    ...
    rhi->kvkCmdPushConstants(command_buffer, pipeline_array[internal_shader->bound_pipeline_index]->pipeline_layout,
        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, 128, internal_shader->per_draw_push_constant_block);
...
b8 vulkan_buffer_bind(renderer_backend_interface* backend, renderbuffer* buffer, u64 offset) {
    ...
    vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
    VK_CHECK(rhi->kvkBindBufferMemory(context->device.logical_device, internal_buffer->handle, internal_buffer->memory, offset));
    return true;
}
...
b8 vulkan_buffer_read(renderer_backend_interface* backend, renderbuffer* buffer, u64 offset, u64 size, void** out_memory) {
    ...
    if (vulkan_buffer_is_device_local(backend, internal_buffer) && !vulkan_buffer_is_host_visible(backend, internal_buffer)) {
        ...
        // Map/copy/unmap
        void* mapped_data;
        VK_CHECK(rhi->kvkMapMemory(context->device.logical_device, read_internal->memory, 0, size, 0, &mapped_data));
...
b8 vulkan_buffer_draw(renderer_backend_interface* backend, renderbuffer* buffer, u64 offset, u32 element_count, b8 bind_only) {
    ...
    if (buffer->type == RENDERBUFFER_TYPE_VERTEX) {
        ...
        rhi->kvkCmdBindVertexBuffers(command_buffer->handle, 0, 1, &((vulkan_buffer*)buffer->internal_data)->handle, offsets);
        if (!bind_only) {
        ...
        rhi->kvkCmdBindIndexBuffer(command_buffer->handle, ((vulkan_buffer*)buffer->internal_data)->handle, offset, VK_INDEX_TYPE_UINT32);
        if (!bind_only) {
...
static b8 vulkan_graphics_pipeline_create(vulkan_context* context, const vulkan_pipeline_config* config, vulkan_pipeline* out_pipeline) {
    ...
    VK_CHECK(rhi->kvkCreatePipelineLayout(context->device.logical_device, &pipeline_layout_create_info, context->allocator, &out_pipeline->pipeline_layout));
    ...
    VkResult result = rhi->kvkCreateGraphicsPipelines(context->device.logical_device, VK_NULL_HANDLE, 1, &pipeline_create_info, context->allocator, &out_pipeline->handle);
...
static b8 frequency_has_uniforms(vulkan_shader_frequency_info* frequency_info) {
    return frequency_info->uniform_count > 0 || frequency_info->uniform_texture_count > 0 || frequency_info->uniform_sampler_count > 0;
}
