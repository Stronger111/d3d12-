...
// For runtime kshader compilation.
...
b8 vulkan_renderer_backend_initialize(renderer_backend_interface* backend, const renderer_backend_config* config) {
    ...
    VkExtensionProperties* available_extensions = darray_reserve(VkExtensionProperties, avaliable_extension_count);
    vkEnumerateInstanceExtensionProperties(0, &avaliable_extension_count, available_extensions);
...
void vulkan_renderer_backend_shutdown(renderer_backend_interface* backend) {
    ...
    // Destroy the runtime kshader compiler
        ...
        for (u8 i = 0;i < max_frames_in_flight;++i) {
          ...
          // until a frame is "rendered" before it.
           ...
           // TODO: Reduce this to a single buffer split by max_frames_in_flight.
            ...
            //Create the pre-frame list of updated texture handles.
...
b8 vulkan_renderer_frame_submit(struct renderer_backend_interface* backend, struct frame_data* p_frame_data) {
    ...
    VkResult result = vkQueueSubmit(context->device.graphics_queue, 1, &submit_info, window_backend->in_flight_fences[window_backend->current_frame]);
...
void vulkan_renderer_winding_set(struct renderer_backend_interface* backend, renderer_winding winding) {
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT) {
        ...
        KFATAL("vulkan_renderer_winding_set cannot be used on a device without dynamic state support.");
    }
}
...
void vulkan_renderer_clear_colour_set(renderer_backend_interface* backend, vec4 clear_colour) {
    ...
    //Clamp values
    for (u8 i = 0;i < 4;++i) {
        clear_colour.elements[i] = KCLAMP(clear_colour.elements[i], 0.0f, 1.0f);
    }

    //Cache the clear colour for the next colour clear operation.
    kcopy_memory(context->colour_clear_value.float32, clear_colour.elements, sizeof(f32) * 4);
}
...
static void create_command_buffers(vulkan_context* context, kwindow* window) {
    ...
    //Create new command buffers according to the swapchain image count.
...
static b8 recreate_swapchain(renderer_backend_interface* backend, kwindow* window) {
    ...
    // Detect if the window is too small to drawn to
...
b8 vulkan_renderer_texture_write_data(renderer_backend_interface* backend, khandle renderer_texture_handle, u32 offset, u32 size, const u8* pixels, b8 include_in_frame_workload) {
    vulkan_context* context = (vulkan_context*)backend->internal_context;
    // Ensure the handle isn't stale.
    vulkan_texture_handle_data* texture_data = &context->textures[renderer_texture_handle.handle_index];
    if (texture_data->uniqueid != renderer_texture_handle.unique_id.uniqueid) {
    ...
    for (u32 i = 0;i < texture_data->image_count;++i) {
        ...
        if (image->mip_levels <= 1 || !vulkan_image_mipmaps_generate(context, image, command_buffer)) {
            ...
            // Transition from optimal for data reciept to kshader-read-only optimal layout.
            // Transition from optimal for data reciept to kshader-read-only optimal layout.
            vulkan_image_transition_layout(
                context,
                command_buffer,
...
static b8 texture_read_offset_range(renderer_backend_interface* backend, vulkan_texture_handle_data* texture_data, u32 offset, u32 size,
    u32 x, u32 y, u32 width, u32 height, u8** out_memory) {
    vulkan_context* context = (vulkan_context*)backend->internal_context;
    if (texture_data) {
        ...
        // Transition from optimal for data reading to kshader-read-only optimal layout.
...
b8 vulkan_renderer_shader_create(renderer_backend_interface* backend, kshader* s, const shader_config* config) {
    ...
    // Note that can mean that only one (or potentially none) exist as well.
    ...
    if (has_per_frame) {
        ...
        // Total bindings are 1 UBO for per_frame(if need), plus per_frame sampler count.
        // This is dynamilly allocated. now
        ...
        if (s->per_frame.uniform_sampler_count > 0) {
            for (u32 i = 0; i < s->per_frame.uniform_sampler_count; ++i) {
                ...
                set_config->bindings[per_frame_binding_index].binding = per_frame_binding_index;
                // Either treat as an array or a single texture. depending on what is passed in.
                set_config->bindings[per_frame_binding_index].descriptorCount = KMAX(u->array_length, 1);
    ...
    if (has_per_group) {
        ...
        if (s->per_group.uniform_sampler_count > 0) {
            for (u32 i = 0; i < s->per_group.uniform_sampler_count; ++i) {
                ...
                set_config->bindings[per_group_binding_index].binding = per_group_binding_index;
                // Either treat as an array or a single texture, depending on what is passed in.
                set_config->bindings[per_group_binding_index].descriptorCount = KMAX(u->array_length, 1);
...
void vulkan_renderer_shader_destroy(renderer_backend_interface* backend, kshader* s) {
    ...
    if (s && s->internal_data) {
        ...
        if (!internal_shader) {
            KERROR("vulkan_renderer_shader_destroy requires a valid pointer to a kshader.");
            return;
        }
        vulkan_context* context = (vulkan_context*)backend->internal_context;
...
static b8 shader_create_modules_and_pipelines(renderer_backend_interface* backend, kshader* s) {
    ...
    for (u32 i = 0; i < internal_shader->stage_count; ++i) {
        ...
        if (!create_shader_module(context, s, sc->stage, sc->source, sc->filename, &new_stages[i])) {
            KERROR("Unable to create %s kshader module for '%s'. Shader will be destroyed.", s->stage_configs[i].filename, s->name);
            has_error = true;
    ...
    for (u32 i = 0; i < pipeline_count; ++i) {
        ...
        pipeline_config.attributes = internal_shader->attributes;  // kshader->attributes,
        ...
        if ((s->flags & SHADER_FLAG_COLOUR_READ) || (s->flags & SHADER_FLAG_COLOUR_WRITE)) {
            ...
            // until there is a kshader with more than 1 colour attachment, in which case either the
            // kshader configuration itself will have to be amended to indicate this directly and/or the
            // kshader configuration can specify some known "pipeline type" (i.e. "forward"), and that
            // type contains the image format information needed here. Putting a pin in this for now
            // until the eventual kshader refactoring.
        ...
        if (!pipeline_result) {
            KERROR("Failed to load graphics pipeline for kshader %s.", s->name);
            has_error = true;
            break;
        }
    }
    ...
    // Destroy the old kshader modules and copy over the new ones.
    for (u32 i = 0; i < internal_shader->stage_count; ++i) {
        vkDestroyShaderModule(context->device.logical_device, internal_shader->stages[i].handle, context->allocator);
...
b8 vulkan_renderer_shader_initialize(renderer_backend_interface* backend, kshader* s) {
    ...
    if (!context->device.features.fillModeNonSolid) {
        KINFO("Renderer backend does not support fillModeNonSolid. Wireframe mode is not possible,but was requested for the kshader '%s'.", s->name);
    ...
    // If this isn't supported,perhaps a different backend should be used.
    u32 pipeline_count = 3;
    // Create an array of pointers to pipelines, one per topology class. Null means not supported for this kshader.
    ...
    if (!shader_create_modules_and_pipelines(backend, s)) {
        KERROR("Failed initial load on kshader '%s'. See logs for details.", s->name);
        return false;
    }
        ...
        if (internal_shader->pipelines[i]) {
            ...
            for (u32 j = 1; j < PRIMITIVE_TOPOLOGY_TYPE_MAX; j = j << 1) {
                if (internal_shader->pipelines[i]->supported_topology_types & j) {
                    ...
                    // whatever is using the kshader if need be.
                    break;
                }
            }
            pipeline_found = true;
    ...
    if (!pipeline_found) {
        ...
        KERROR("No available topology classes are available, so a pipeline cannot be bound. Check kshader configuration.");
        return false;
    }

    // Grab the UBO alignment requirement from the device. Debug一下
    ...
    for (u32 i = 0;i < image_count;++i) {
        ...
        if (!renderer_renderbuffer_create(buffer_name, RENDERBUFFER_TYPE_UNIFORM, total_buffer_size, RENDERBUFFER_TRACK_TYPE_FREELIST, &internal_shader->uniform_buffers[i])) {
            KERROR("Vulkan buffer creation failed for object kshader.");
...
static b8 vulkan_descriptorset_update_and_bind(
    ...
    if (needs_update) {
        // Alocate enough descriptor writes to handle the max allowed bound textures.
       ...
       // UBO, then samplers (if they exist) and finally textures (if they exist).
        ...
        if (sampler_count > 0) {
            ...
            for (u32 k = 0; k < sampler_count; ++k) {
                vulkan_uniform_sampler_state* binding_sampler_state = &samplers[k];
                ...
                binding_image_infos[k] = p_frame_data->allocator.allocate(sizeof(VkDescriptorImageInfo) * binding_descriptor_count);
                ...
                for (u32 d = 0; d < binding_descriptor_count; ++d) {
                    ...
                    //Not using image.
                    binding_image_infos[k][d].imageLayout = VK_IMAGE_LAYOUT_UNDEFINED;
                    binding_image_infos[k][d].imageView = 0;
                    binding_image_infos[k][d].sampler = sampler->sampler;

                    update_sampler_count++;


                }
                // 把Texture 资源绑定到 Sampler描述符上
                ...
                sampler_descriptor.pImageInfo = binding_image_infos[k];
        ...
        if (descriptor_write_count > 0) {
            // TODO: This can (and probaby should be) split out to a separate frame_prepare step from the
    ...
    // Bind the descriptor set to be updated, or in case the kshader changed.
...
b8 vulkan_renderer_shader_apply_per_frame(renderer_backend_interface* backend, kshader* s, u64 renderer_frame_number) {
    ...
    // Global is always first, if exists.
    ...
    if (!vulkan_descriptorset_update_and_bind(
        ...
        KERROR("Failed to update/bind per-frame descriptor.");
        return false;
    }
    return true;
}
...
b8 vulkan_renderer_shader_apply_per_draw(renderer_backend_interface* backend, struct kshader* s, u64 renderer_frame_number) {
    vulkan_context* context = (vulkan_context*)backend->internal_context;
    vulkan_shader* internal = s->internal_data;
    VkCommandBuffer command_buffer = get_current_command_buffer(context)->handle;
    ...
    vkCmdPushConstants(command_buffer, pipeline_array[internal->bound_pipeline_index]->pipeline_layout,
        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, 128, internal->per_draw_push_constant_block);
    ...
    if (s->per_draw.uniform_sampler_count > 0) {
        ...
        descriptor_set_index += has_per_group ? 1 : 0;
        ...
        if (!vulkan_descriptorset_update_and_bind(
            ...
            KERROR("Failed to update/bind  per-draw  sampler descriptor.");
            return false;
        }
    }
    return true;
...
b8 vulkan_renderer_shader_per_group_resources_acquire(renderer_backend_interface* backend, struct kshader* s, u32* out_group_id) {
    vulkan_context* context = (vulkan_context*)backend->internal_context;
    return setup_frequency_state(context, s, SHADER_UPDATE_FREQUENCY_PER_GROUP, out_group_id);
}

b8 vulkan_renderer_shader_per_draw_resources_acquire(renderer_backend_interface* backend, struct kshader* s, u32* out_per_draw_id) {
    vulkan_context* context = (vulkan_context*)backend->internal_context;
    ...
    for (u32 i = 0; i < texture_count; ++i) {
        ...
        if (texture_state->uniform.location == uniform_location) {
            ...
            if (index >= texture_state->uniform.array_length) {
                KERROR("vulkan_renderer_uniform_set error: array_index (%u) is out of range (0-%u)", index, texture_state->uniform.array_length);
                return false;
            }
...
b8 vulkan_renderer_uniform_set(renderer_backend_interface* backend, kshader* s, shader_uniform* uniform, u32 array_index, const void* value) {
    ...
    if (uniform_type_is_texture(uniform->type)) {
        ...
        case SHADER_UPDATE_FREQUENCY_PER_DRAW:
            if (s->per_draw.bound_id == INVALID_ID) {
                KERROR("An per_draw id must be bound before setting a per_draw uniform.");
                return false;
            }
        ...
        case SHADER_UPDATE_FREQUENCY_PER_GROUP:
            if (s->per_group.bound_id == INVALID_ID) {
                KERROR("An per_group id must be bound before setting an instance uniform.");
                return false;
            }
...
static b8 create_shader_module(vulkan_context* context, kshader* s, shader_stage stage, const char* source, const char* filename, vulkan_shader_stage* out_stage) {
    ...
    case SHADER_STAGE_GEOMETRY:
        ...
        break;
    default:
        KERROR("Unsupported kshader kind.Unable to create module.");
        return false;
    }

    KDEBUG("Compiling stage '%s' for kshader '%s'... ", shader_stage_to_string(stage), s->name);
    ...
    if (status != shaderc_compilation_status_success) {
        ...
        KERROR("Error compiling kshader with %llu errors.", error_count);
        KERROR("Error(s):\n%s", error_message);
...
static b8 vulkan_graphics_pipeline_create(vulkan_context* context, const vulkan_pipeline_config* config, vulkan_pipeline* out_pipeline) {
    ...
    if (config->shader_flags & SHADER_FLAG_STENCIL_TEST) {
        ...
        depth_stencil.back.reference = 1;  //将所有片段都要写入模版缓冲
        depth_stencil.back.compareMask = 0xFF; //设置模版缓冲为可写状态
...
static b8 setup_frequency_state(vulkan_context* context, kshader* s, shader_update_frequency frequency, u32* out_frequency_id) {
    ...
    case SHADER_UPDATE_FREQUENCY_PER_GROUP:
        ...
        uniform_texture_count = s->per_group.uniform_count;
    ...
    if (frequency != SHADER_UPDATE_FREQUENCY_PER_FRAME) {
        ...
        *out_frequency_id = INVALID_ID;
        ...
        if (*out_frequency_id == INVALID_ID) {
            KERROR("setup_frequency_state failed to acquire new %s id for kshader '%s', max %s count=%u", frequency_text, s->name, frequency_text, max_frequency_count);
            return false;
        }
    ...
    if (do_ubo_setup) {
        ...
        for (u32 j = 0; j < image_count; ++j) {
            ...
            frequency_state->ubo_descriptor_state.ids[j] = INVALID_ID;
    ...
    for (u32 i = 0; i < image_count; ++i) {
        char* desc_set_object_name = string_format("desc_set_shader_%s_instance_%u_frame_%u", s->name, *out_frequency_id, i);
...
static b8 release_frequency_state(vulkan_context* context, kshader* s, shader_update_frequency frequency, u32 frequency_id) {
    ...
    if (result != VK_SUCCESS) {
        KERROR("Error freeing %s kshader descriptor sets!", frequency_text);
    }
        ...
        if (ubo_stride != 0) {
            for (u32 i = 0; i < internal->uniform_buffer_count; ++i) {
                if (!renderer_renderbuffer_free(&internal->uniform_buffers[i], ubo_stride, frequency_state->offset)) {
                    KERROR("vulkan_renderer_shader_instance_resources_release failed to free range from renderbuffer.");
                }
            }
        }
    }
