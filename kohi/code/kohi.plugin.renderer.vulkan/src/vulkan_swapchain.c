...
static b8 create(renderer_backend_interface* backend, kwindow* window, renderer_config_flags flags, vulkan_swapchain* swapchain) {
    ...
    //Query swapchain image format properties to see if it can be a src/destination for bitting.
    ...
    swapchain->supports_blit_dest = (format_properties.optimalTilingFeatures & VK_FORMAT_FEATURE_TRANSFER_DST_BIT) != 0;
    swapchain->supports_blit_src = (format_properties.optimalTilingFeatures & VK_FORMAT_FEATURE_TRANSFER_SRC_BIT) != 0;
    ...
    //Of course, only do this if hasn't already been done.
    ...
    //Swapchain images are stored in the backend of the swapchain's colour texture.
    //NOTE: The window should create a seprate set of images to render to, then blit to these.
