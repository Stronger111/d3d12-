...
b8 render_view_world_on_registered(struct render_view* self) {
    if (self) {
        ...
        data->skybox_locations.cube_map_locations = shader_system_uniform_index(data->skybox_shader, "cube_texture");
        ...
        // Get colour3d shader uniform location.
        {
...
b8 render_view_world_on_packet_build(const struct render_view* self, struct frame_data* frame_data, viewport* v, void* data, struct render_view_packet* out_packet) {
    if (!self || !data || !out_packet) {
        KWARN("render_view_world_on_packet_build requires valid pointer to view, packet, and data.");
        return false;
    }
...
void render_view_world_on_packet_destroy(const render_view* self, render_view_packet* packet) {
...
b8 render_view_world_on_render(const struct render_view* self, const struct render_view_packet* packet, const struct frame_data* p_frame_data) {
    ...
    // Bind renderpass.
    {
        ...
        if (!renderer_renderpass_begin(pass, &pass->targets[p_frame_data->render_target_index])) {
            KERROR("render_view_world_on_render pass failed to start.");
            return false;
        }
        ...
        if (packet->skybox_data.sb) {
            ...
            if (!shader_system_uniform_set_by_index(data->skybox_locations.cube_map_locations, &packet->skybox_data.sb->cubemap)) {
                KERROR("Failed to apply skybox view uniform.");
                return false;
            }
            b8 needs_update = packet->skybox_data.sb->render_frame_number != p_frame_data->renderer_frame_number || packet->skybox_data.sb->draw_index !=
                                                                                                                        p_frame_data->draw_index;
    ...
    // World renderpass
    {
        ...
        if (!renderer_renderpass_begin(pass, &pass->targets[p_frame_data->render_target_index])) {
            KERROR("render_view_world_on_render world pass to start.");
            return false;
        }
        ...
        // TODO: Debug geometries (i.e grids,lines,boxs,gizmos,etc.)
...
// 快速排序
void quick_sort(geometry_distance arr[], i32 low_index, i32 high_index, b8 ascending) {
