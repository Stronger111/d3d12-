...
shader_update_frequency string_to_shader_update_frequency(const char* str) {
    if (strings_equali("frame", str)) {
        ...
        KERROR("Unknown shader update frequency '%s'. Defaulting to per draw.", str);
        return SHADER_UPDATE_FREQUENCY_PER_FRAME;
    }
}
...
const char* material_map_channel_to_string(kasset_material_texture_map channel) {
    switch (channel) {
    default:
...
kasset_material_texture_map string_to_material_map_channel(const char* str) {
