...
KAPI void freelist_create(u64 total_size, u64* memory_requirement, void* memory, freelist* out_list) {
    ...
    // Catch an edge case of having a readlly small amount of memory to manager, and only having a
    // super small of entries. Always make sure we have at least a decent amount,like 20 or so.
    if (max_entries < 20) {
        max_entries = 20;
    }

    *memory_requirement = sizeof(internal_state) + (max_entries * sizeof(freelist_node));
    if (!memory) {
        return;
    }
...
KAPI b8 freelist_resize(freelist* list, u64* memory_requirement, void* new_memory, u64 new_size, void** out_old_memory) {
    ...
    *memory_requirement = sizeof(internal_state) + (sizeof(freelist_node) * max_entries);
    if (!new_memory) {  // 第一次获取到memory_requirement  new_memory传入0
        return true;
    }
