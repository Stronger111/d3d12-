...
k_handle kregistry_add_entry(kregistry* registry, const void* block, u64 size, b8 auto_release) {
    if (!registry || !size) {
        KERROR("kregistry_add_entry requires a valid pointer to registry, and  a nonzero size, Invalid handle will be returned.");
...
b8 kregistry_entry_set(kregistry* registry, k_handle entry_handle, const void* block, u64 size, void* sender) {
    if (!registry || !block || !size) {
        KERROR("kregistry_entry_set requires a valid pointer to registry, and  a nonzero size, Nothing was done.");
        return false;
    }

    if (k_handle_is_invalid(entry_handle)) {
        KERROR("kregistry_entry_set  requires a valid handle, yet an invalid one was passed. Nothing was done.");
        return false;
    }
...
b8 kregistry_entry_update_callback_for_listener(kregistry* registry, k_handle entry_handle, void* listener, PFN_on_registry_entry_updated updated_callback) {
    if (!registry || !listener || !updated_callback) {
        KERROR("kregistry_entry_update_callback_for_listener requires a valid pointer to registry, listener,and updated_callback. Nothing was done.");
        return false;
    }
