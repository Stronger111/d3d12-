...
void* _darray_create(u64 length, u64 stride, frame_allocator_int* allocator) {
    ...
    return (void*)((u8*)new_array + header_size);  // 数组的下标开始 指针向前24个字节
}
...
void* _darray_push(void* array, const void* value_ptr) {
    ...
    kcopy_memory((void*)addr, value_ptr, header->stride);  // 内存复制
...
void* _darray_duplicate(u64 stride, void* array) {
    ...
    //copy data
    kcopy_memory(copy, array, source_header->length * source_header->stride);
