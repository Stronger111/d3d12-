...
static b8 extract_input_map_channel_or_vec4(const kson_object* inputs_obj, const char* input_name, b8* out_enabled, kmaterial_texture_input* out_map, vec4* out_value, vec4 default_value);
static b8 extract_input_map_channel_or_vec3(const kson_object* inputs_obj, const char* input_name, b8* out_enabled, kmaterial_texture_input* out_map, vec3* out_value, vec3 default_value);
...
const char* kasset_material_serialize(const kasset* asset) {
    ...
    //Various flags
    kson_object_value_add_bool(&tree.root, "has_transparency", material->has_transparency);
    kson_object_value_add_bool(&tree.root, "double_sided", material->double_sided);
    kson_object_value_add_bool(&tree.root, "recieves_shadow", material->recieves_shadow);
    kson_object_value_add_bool(&tree.root, "casts_shadow", material->casts_shadow);
    kson_object_value_add_bool(&tree.root, "use_vertex_colour_as_base_colour", material->use_vertex_colour_as_base_colour);
    ...
    if (material->custom_samplers && material->custom_sampler_count) {
        ...
        for (u32 i = 0;i < material->custom_sampler_count;++i) {
            ...
            kson_object_value_add_string(&sampler, "filter_min", texture_filter_to_string(custom_sampler->filter_min));
            kson_object_value_add_string(&sampler, "filter_mag", texture_filter_to_string(custom_sampler->filter_mag));
...
b8 kasset_material_deserialize(const char* file_text, kasset* out_asset) {
    ...
    if (file_format_version != MATERIAL_FILE_VERSION) {
        if (file_format_version < 3) {
            KERROR("Material file format version %u no longer supported. File should be manually converted to at least version %u. Material will not be processed.", file_format_version, MATERIAL_FILE_VERSION);KERROR("Material file format version %u no longer supported. File should be manually converted to at least version 3. Material will not be processed.", file_format_version);
            goto cleanup;
        }
