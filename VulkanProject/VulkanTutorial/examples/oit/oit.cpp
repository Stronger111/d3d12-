...
public:
	...
	void getEnabledFeatures() override
	{
		...
		{
			vks::tools::exitFatal("Selected GPU does not suport stores and atomic operations in the fragment stage", VK_ERROR_FEATURE_NOT_PRESENT);
		}
	}
...
private:
	...
	void prepareUniformBuffers()
	{
		...
		VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
			&uniformBuffers.renderPass, sizeof(renderPassUBO)));
	...
	void prepareGeometryPass()
	{
		...
		VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT
			| VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffer, sizeof(geometrySBO)));
	   ...
	   //This image will track the head index of each fragment.
		...
		VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
			&geometryPass.linkedList, sizeof(Node) * geometrySBO.maxNodeCount));
