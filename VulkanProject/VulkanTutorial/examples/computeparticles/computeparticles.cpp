...
public:
	...
	struct
	{
		...
		VkDescriptorSet descriptorSet; //Particle system rendering shader binding
	...
	struct
	{
		...
		VkCommandBuffer commandBuffer; //Command buffer storing the dispatch commnands and barriers
		VkSemaphore semaphore; //Execution dependency between compute & graphic submission
		VkDescriptorSetLayout descriptorSetLayout; //Compute shader binding layout
		VkDescriptorSet descriptorSet; //Compute shader binding
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			if (graphics.queueFamilyIndex != compute.queueFamilyIndex)
			{
				...
				vkCmdPipelineBarrier(drawCmdBuffers[i], VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
	...
	void buildComputeCommandBuffer()
	{
		...
		if (graphics.queueFamilyIndex != compute.queueFamilyIndex)
		{
			...
			vkCmdPipelineBarrier(compute.commandBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
	...
	void prepareStorageBuffers()
	{
		...
		if (graphics.queueFamilyIndex != compute.queueFamilyIndex)
		{
			...
			vkCmdPipelineBarrier(copyCmd, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
				VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, nullptr, 1, &buffer_barrier, 0, nullptr);
		}
		...
		//Location 0 :Position
		vertices.attributeDescriptions[0] = vks::initializers::vertexInputAttributeDescription(VERTEX_BUFFER_BIND_ID,
			0, VK_FORMAT_R32G32_SFLOAT, offsetof(Particle, pos));
		//Location 1 : Gradient position
		vertices.attributeDescriptions[1] = vks::initializers::vertexInputAttributeDescription(VERTEX_BUFFER_BIND_ID,
			1, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(Particle, gradientPos));
	...
	void setupDescriptorPool()
	{
		...
		VkDescriptorPoolCreateInfo descriptorPoolInfo = vks::initializers::descriptorPoolCreateInfo(static_cast<uint32_t>(poolSize.size()), poolSize.data(), 2);
	...
	void setupDescriptorSetLayout()
	{
		...
		setLayoutBindings.push_back(vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
			VK_SHADER_STAGE_FRAGMENT_BIT, 0));
		//Binding 1 : Particle gradient ramp
		setLayoutBindings.push_back(vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
			VK_SHADER_STAGE_FRAGMENT_BIT, 1));

		VkDescriptorSetLayoutCreateInfo descriptorLayout = vks::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings.data(),
	...
	void setupDescriptorSet()
	{
		...
		// Binding 0 : Particle color map
		writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(graphics.descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
			0, &textures.particle.descriptor));
	...
	void prepareCompute()
	{
		...
		{
			// Binding 0 : Particle position storage buffer
			vks::initializers::writeDescriptorSet(compute.descriptorSet,VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,0,&compute.storageBuffer.descriptor),
			//Binding 1 : Uniform buffer
