...
public:
	...
	//shader Binding 0 顶点着色器绑定点和片元着色器上的绑定点
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			//Binding point 0 : Mesh vertex bufffer
	...
	void preparePipelines()
	{
		...
		//Note that the shader declaration for per-vertex and per-instance attribute is the same,the different input rates are only stored in the bindings:
		//instanced vert:
		...
		{
			//Per-vertex attributes Mesh顶点属性
	...
	void prepareIndirectData()
	{
		...
		VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
			...
			&stagingBuffer, indirectCommands.size() * sizeof(VkDrawIndexedIndirectCommand), indirectCommands.data()));

		VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT
		| VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,&indirectCommandsBuffer,stagingBuffer.size));
	...
	void prepareUniformBuffers()
	{
		VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT| VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
			&uniformData.scene,sizeof(uboVS)));
