...
public:
	...
	//Pool that stores all occlusion queries 存储所有的查询结果
	...
	//Retrieves the results of the occlusion queies submitted to the command buffer
	void getQueryResults()
	{
		...
		vkGetQueryPoolResults(
			...
			//If you don't want to wait, yout can use VK_QUERY_RESULT_WITH_AVAILABILITY_BIT
			//which alse returns the state of the result (ready) in the result
	...
	void setupDescriptorPool()
	{
		...
		VkDescriptorPoolCreateInfo descriptorPoolInfo = vks::initializers::descriptorPoolCreateInfo(poolSizes.size(),
			poolSizes.data(),3);
	...
	void setupDescriptorSets()
	{
		...
		std::vector<VkWriteDescriptorSet> writeDescriptorSets =
		{
			...
			vks::initializers::writeDescriptorSet(
				descriptorSet,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,0,&uniformBuffers.occluder.descriptor
			)
		};
