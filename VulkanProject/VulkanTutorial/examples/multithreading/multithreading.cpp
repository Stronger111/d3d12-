...
public:
	...
	//by using threads and secondary commnand buffers
	...
	void prepareMultiThreadedRenderer()
	{
		...
		VkCommandBufferAllocateInfo cmdBufferAllocateInfo = vks::initializers::commandBufferAllocateInfo(cmdPool,
			VK_COMMAND_BUFFER_LEVEL_PRIMARY, 1);
		VK_CHECK_RESULT(vkAllocateCommandBuffers(device, &cmdBufferAllocateInfo, &primaryCommandBuffer));

		//Create additional secondary CBs for background and ui
		cmdBufferAllocateInfo.level = VK_COMMAND_BUFFER_LEVEL_SECONDARY;
		VK_CHECK_RESULT(vkAllocateCommandBuffers(device, &cmdBufferAllocateInfo, &secondaryCommandBuffers.background));
		VK_CHECK_RESULT(vkAllocateCommandBuffers(device, &cmdBufferAllocateInfo, &secondaryCommandBuffers.ui));
		//resset size
		...
		for (uint32_t i = 0; i < numThreads; i++)
		{
			...
			VkCommandBufferAllocateInfo secondaryCmdBufAllocateInfo = vks::initializers::commandBufferAllocateInfo(thread
				->commandPool, VK_COMMAND_BUFFER_LEVEL_SECONDARY, thread->commandBuffer.size());
	...
	void updateSecondaryCommandBuffers(VkCommandBufferInheritanceInfo inheritanceInfo)
	{
		...
		//User interface
		//with VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFER,the primary command buffer's content has to be defined
	...
	//Update the secondary command buffers using a thread pool
	...
	void updateCommandBuffers(VkFramebuffer frameBuffer)
	{
		...
		//These are stored(are retrieved) from the secondary command buffers
		...
		//Execute render commands from the secondary command bufffer
