...
public:
	...
	void preparePipelines()
	{
		...
		attributeDescriptions =
		{
			...
			vks::initializers::vertexInputAttributeDescription(INSTANCE_BUFFER_BIND_ID,7,VK_FORMAT_R32_SINT,sizeof(float)*7), //Location 7: TextureIndex
		};
	...
	void prepareInstanceData()
	{
		...
		// This results in better performance  优化性能？ 本地Memery
		struct 
		{
			VkDeviceMemory memory;
			VkBuffer buffer;
		}stagingBuffer;
		//对于一个资源的理想的内存放置区域可能是一块host不可见的内存，那么在这种情况下，App就需要先在host-visible的staging buffer这个阶段把resource的数据或者字段填充好
		//，然后再把这个resource转到理想的device local的的内存区域里。
		...
		VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,instanceBuffer.size,&instanceBuffer.buffer,&instanceBuffer.memory));
