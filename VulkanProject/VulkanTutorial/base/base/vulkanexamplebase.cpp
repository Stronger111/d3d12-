...
bool VulkanExampleBase::initVulkan()
{
	...
	if (err)
	{
		vks::tools::exitFatal("Cound not enumerate physical devices : \n" + vks::tools::errorString(err), err);
	}
	return false;
...
{
	typedef HRESULT* (__stdcall* SetPrcessDpiAwarenessFunc)(PROCESS_DPI_AWARENESS);

	HMODULE shCore = LoadLibraryA("Shcore.dll");
	if (shCore)
	{
		SetPrcessDpiAwarenessFunc setProcessDpiAwareness = (SetPrcessDpiAwarenessFunc)GetProcAddress(shCore, "SetProcessDpiAwareness");
...
void VulkanExampleBase::setupFrameBuffer()
{
	...
	for (uint32_t i = 0; i < frameBuffers.size(); i++)
	{
		attachments[0] = swapChain.buffers[1].view;
