...
private:
	...
	*/
	...
	void createTextureSampler()
	{
		...
		samplerInfo.borderColor = VK_BORDER_COLOR_INT_OPAQUE_BLACK; //黑色
	...
	*/
	...
	void generateMipmaps(VkImage image,VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
	{
		...
		for (uint32_t i = 1; i < mipLevels; i++)
		{
			...
			blit.dstOffsets[1] = { mipLevels > 1 ? mipWidth / 2 : 1, mipHeight > 1 ? mipHeight / 2 : 1, 1 };
	...
	*/
	void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
	{
		...
		VkMemoryRequirements menRequirements;
		vkGetBufferMemoryRequirements(device, buffer, &menRequirements);

		VkMemoryAllocateInfo allocInfo{};
		allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
		allocInfo.allocationSize = menRequirements.size;
		allocInfo.memoryTypeIndex = findMemoryType(menRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	...
	*/
	...
	void createGraphicsPipeline() {
		auto vertShaderCode = readFile("Shaders/shader_ubo.spv");
		auto fragShaderCode = readFile("Shaders/09_shader_base_Frag.spv");
